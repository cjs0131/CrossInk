...
inline SettingInfo buildFontFamilySetting(const SdCardFontRegistry* registry) {
  ...
  std::vector<StrId> enumValues = {StrId::STR_LEXEND_DECA, StrId::STR_LATO};
  ...
  if (sdFontCount > 0) {
    ...
    allStringValues.push_back(I18N.get(StrId::STR_LATO));
...
inline const std::vector<SettingInfo>& getBaseSettingsList() {
  static const std::vector<SettingInfo> baseList = [] {
    ...
    add(SettingInfo::Enum(StrId::STR_FONT_FAMILY, &CrossPointSettings::fontFamily,
                          {StrId::STR_LEXEND_DECA, StrId::STR_LATO}, "fontFamily", StrId::STR_CAT_READER));
